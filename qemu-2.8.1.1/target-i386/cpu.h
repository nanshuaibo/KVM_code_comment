 ...
 * An x86 CPU.
 */
//一个X86CPU结构代表一个虚拟x86cpu
struct X86CPU {
    /*< private >*/
    //所有cpu的通用数据，cpu核心数，线程数等，kvm模拟器相关数据，
    //如kvm_fd,kvm_state,kvm_run等kvm相关的数据
    //CPUstate中包含一个env_ptr指针，指向其cpu架构的cpu状态信息
    CPUState parent_obj; 
    /*< public >*/      
    
    //x86架构cpu的寄存器信息，kvm相关的异常和中断信息及cpuid信息
    CPUX86State env;
    ...
    bool hyperv_synic;
    bool hyperv_stimer;
    bool check_cpuid; //指示是否需要检测宿主机的CPUID
    bool enforce_cpuid;
    bool expose_kvm;
    bool migratable;
    bool host_features;
    uint32_t apic_id; //表示cpu对应的lapic的id
