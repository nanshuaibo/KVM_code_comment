...
struct X86CPUDefinition {
    const char *name; //CPU名字
    uint32_t level; //CPUid支持的额最大功能号
...
/*builtin_x86_defs数组每项表示模拟的cpu类型，包括实际的cpu和虚拟机的cpu类型*/
...
static void x86_cpu_register_types(void)
{
    ...
    type_register_static(&x86_cpu_type_info);//注册TYPE_X86_CPU对象 
    for (i = 0; i < ARRAY_SIZE(builtin_x86_defs); i++) {
        x86_register_cpudef_type(&builtin_x86_defs[i]); //构造TypeInfo结构
    }
#ifdef CONFIG_KVM
