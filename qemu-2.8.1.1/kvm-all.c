...
int kvm_init_vcpu(CPUState *cpu
{
    ...
    ret = kvm_get_vcpu(s, kvm_arch_vcpu_id(cpu)); //创建vcpu，返回值为虚拟cpu的fd
    if (ret < 0) {
    ...
    mmap_size = kvm_ioctl(s, KVM_GET_VCPU_MMAP_SIZE, 0);//返回kvm和qemu共享内存的大小
    if (mmap_size < 0) {
        ret = mmap_size;
        DPRINTF("KVM_GET_VCPU_MMAP_SIZE failed\n");
        goto err;
...
int kvm_cpu_exec(CPUState *cpu)
{
    ...
    do {
        ...
        run_ret = kvm_vcpu_ioctl(cpu, KVM_RUN, 0);//通过ioctl让vcpu在物理cpu上运行起来
        ...
        /*vcpu运行起来后，应用层阻塞在此处，当虚拟机产生VM Exit，根据退出原因循环进行处理*/
