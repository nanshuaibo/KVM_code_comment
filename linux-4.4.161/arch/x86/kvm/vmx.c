...
/*对于kvm的实现中，并不是每次迁移开始前才开启PML特性，而是在kvm模块初始化的过程中开启，
PML Buffer的分配是在vcpu创建过程中。迁移结束也不会关闭PML特性。PML特性是一直存在于KVM模块的生命周期中的。*/
...
//vmx的vcpu结构体
struct vcpu_vmx {
	struct kvm_vcpu       vcpu; //通用层面的vcpu 
...
static int vmx_vcpu_setup(struct vcpu_vmx *vmx)
{
	...
	if (enable_pml) {
		ASSERT(vmx->pml_pg);
		vmcs_write64(PML_ADDRESS, page_to_phys(vmx->pml_pg)); //写入PML地址
		vmcs_write16(GUEST_PML_INDEX, PML_ENTITY_NUM - 1); //写入PML bufffer的索引和PML buffer中的条目对应
	}
...
{
	struct vcpu_vmx *vmx = to_vmx(vcpu);
	/*一块内存区域，用来存放上一次开启PML特性之后，CPU写过的
	物理页的地址，大小为4K，可以存放512条GPA，KVM就是通过这个区域来跟踪内存脏页。
	*/
	u64 *pml_buf;

	/*用来保存PML Buffer的索引，长度16 bit，表示范围0-511，
	  和PML Buffer中的条目对应。PML index用来保存下一次CPU
	  写物理页保存GPA时在PML Buffer中的位置。PML index每次递减。
	*/
	u16 pml_idx;
...
{
	int err;
	//从kvm_vcpu_cache中分配vcpu_vmx
	struct vcpu_vmx *vmx = kmem_cache_zalloc(kvm_vcpu_cache, GFP_KERNEL);
	int cpu;

	if (!vmx)
		return ERR_PTR(-ENOMEM);
	//分配vpid，vpid用于开启ept的情况，每个vcpu与一个vpid相关联
	 ...
	 * for the guest, etc.
	 */
	//pml是page modification logging 用于在硬件层面记录虚拟机中访问的物理页面，能够实现快速标记脏页
	if (enable_pml) {
	...
	//指向当前vcpu对应的vmcs区域，vmcs01表示普通虚拟化
	vmx->loaded_vmcs = &vmx->vmcs01;

	//分配vmcs区域，大小由vmcs_config.order决定
	...
	cpu = get_cpu();
	//将perCPU变量current_vmcs设置成刚刚分配的vmcs
