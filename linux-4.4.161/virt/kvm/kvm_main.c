...
int kvm_vcpu_init(struct kvm_vcpu *vcpu, struct kvm *kvm, unsigned id)
{
	...
	//对kvm_vcpu架构相关成员kvm_vcpu_arch进行初始化
	//创建lapic，创建mmu等和架构相关的工作
...
{
	unsigned long dirty_bytes = 2 * kvm_dirty_bitmap_bytes(memslot); //分配两个脏页位图的大小
			    ...
			    const struct kvm_userspace_memory_region *mem)
{
	...
	if ((new.flags & KVM_MEM_LOG_DIRTY_PAGES) && !new.dirty_bitmap) {
		if (kvm_create_dirty_bitmap(&new) < 0)  //创建脏页位图
			goto out_free;
	}
...
{
	if (kvm_arch_vcpu_runnable(vcpu)) {//检查cpu是否是可运行状态
		kvm_make_request(KVM_REQ_UNHALT, vcpu);//解除vcpu的挂起状态
		return -EINTR;
	}
	if (kvm_cpu_has_pending_timer(vcpu))//检查是否有挂起的定时器操作
		return -EINTR;
	if (signal_pending(current)) //检查当前vcpu是否有挂起的信号
		return -EINTR;
...
{
	ktime_t start, cur;//时间戳
	DEFINE_WAIT(wait);//定义等待队列头
	bool waited = false; //标志位，是否发生等待
	u64 block_ns; //记录挂起的时间

	start = cur = ktime_get(); //当前时间
	if (vcpu->halt_poll_ns) { //检查是否设置了轮询等待
		ktime_t stop = ktime_add_ns(ktime_get(), vcpu->halt_poll_ns); //计算阻塞时间

		++vcpu->stat.halt_attempted_poll; //记录轮询次数
		do {
			/*
			...
			if (kvm_vcpu_check_block(vcpu) < 0) { 
				++vcpu->stat.halt_successful_poll;//记录轮询成功的次数 
				goto out;
			}
			cur = ktime_get();
	...
	kvm_arch_vcpu_blocking(vcpu); //挂起vcpu

	for (;;) {
		prepare_to_wait(&vcpu->wq, &wait, TASK_INTERRUPTIBLE); //将当前vcpu进程加入等待队列
	...
	kvm_arch_vcpu_unblocking(vcpu);//解除挂起操作
out:
	block_ns = ktime_to_ns(cur) - ktime_to_ns(start); //计算阻塞时间

	if (halt_poll_ns) {
		if (block_ns <= vcpu->halt_poll_ns)
			;
		/* we had a long block, shrink polling 阻塞时间过长 缩小轮询时间*/
		else if (vcpu->halt_poll_ns && block_ns > halt_poll_ns)
			shrink_halt_poll_ns(vcpu);
		/* we had a short halt and our poll time is too small   阻塞时间较短，且轮询时间过小*/
		else if (vcpu->halt_poll_ns < halt_poll_ns &&
			block_ns < halt_poll_ns)
			grow_halt_poll_ns(vcpu); //增加轮询时间
	} else
		vcpu->halt_poll_ns = 0;
...
static int kvm_vm_ioctl_create_vcpu(struct kvm *kvm, u32 id)
{
	...
	mutex_lock(&kvm->lock);
	//确保虚拟机的中断芯片组合与vcpu的lapic一致，要么都在qemu中实现，要么都在kvm中实现
	...
	//应用层qemu通过fd对vcpu操作，所有vcpu都保存在表示虚拟机的kvm的vcpus成员中
			  ...
			  unsigned int ioctl, unsigned long arg)
{
	...
	/*返回大小可能为1个、2个、3个页*/
		if (arg)
			goto out;
		r = PAGE_SIZE;     /*第一页用于kvm_run struct kvm_run */
#ifdef CONFIG_X86
		r += PAGE_SIZE;    /*第二页用于虚拟机访问io端口时存储相应的数据 pio data page */
#endif
#ifdef KVM_COALESCED_MMIO_PAGE_OFFSET
		r += PAGE_SIZE;    /*第三页用于聚合mmio coalesced mmio ring page */
#endif
		break;
