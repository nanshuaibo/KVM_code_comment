...
static int (*kvm_vmx_exit_handlers[])(struct kvm_vcpu *vcpu) = {
	...
	[EXIT_REASON_APIC_ACCESS]             = handle_apic_access,  //读写访问LAPIC寄存器的处理函数
	[EXIT_REASON_APIC_WRITE]              = handle_apic_write,   //写lapic寄存器处理函数，打开virtual-APIC page特性后,读不在触发vm exit
	...
	[EXIT_REASON_EPT_VIOLATION]	      = handle_ept_violation, //ept页错误
	[EXIT_REASON_EPT_MISCONFIG]           = handle_ept_misconfig, //mmio页错误
...
static fastpath_t vmx_vcpu_run(struct kvm_vcpu *vcpu)
{
	...
	/*（VMENTER/EXIT）放在.noinstr.text段中*/
